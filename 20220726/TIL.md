...
### 1. 문자열
문자들의 나열로, 모든 문자는 str타입이며 변경이 불가능한 immutable이다. 
(c처럼 char형이 모여서 str 배열이 되는 형식이 아님.)  
변경이 불가능하다는것은  
...
```
(3) `s.isalpha()` 
　알파벳 문자 여부 확인 True, False 로 반환 (단순 알파벳이 아니라, 유니코드상. 한국어도 포함)  
　'abc' , 'ㄱㄴㄷ' 모두 True.    
　'a!' , ' ' , '0' 는 모두 False  
...
집합 개념을 이용하는 자료형이다. 변경 및 추가는 가능(가변형)  
...
+++ isdisjoint는 세트뿐만아니라 순회형이라면 모두 사용가능하다. 
　즉 리스트 딕셔너리 튜플 문자열도 가능.  
